...
public class CallstackTransferRelation extends SingleEdgeTransferRelation {
    ...
    public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(AbstractState pElement, Precision pPrecision, CFAEdge pEdge) throws CPATransferException {
        ...
        switch (pEdge.getEdgeType()) {
            ...
            case FunctionCallEdge: {
                ...
                if (hasRecursion(e, calledFunction)) {
                    if (skipRecursiveFunctionCall(e, (FunctionCallEdge) pEdge)) {
                        ...
                        logger.logOnce(Level.WARNING, "Skipping recursive function call from", pred.getFunctionName(), "to", calledFunction);
                        return ImmutableSet.of();
                    } else {
            ...
            case FunctionReturnEdge: {
                ...
                assert calledFunction.equals(e.getCurrentFunction()) || isWildcardState(e, AnalysisDirection.FORWARD) : String.format("not in scope of called function \"%s\" when leaving function \"%s\" in state \"%s\"", calledFunction, e.getCurrentFunction(), e);
                ...
                if (isWildcardState(e, AnalysisDirection.FORWARD)) {
                    ...
                    assert callerFunction.equals(returnElement.getCurrentFunction()) || isWildcardState(returnElement, AnalysisDirection.FORWARD) : String.format("calling function \"%s\" not available after function return into function scope \"%s\" in state \"%s\"", callerFunction, returnElement.getCurrentFunction(), returnElement);
                }
    ...
    protected boolean skipRecursiveFunctionCall(final CallstackState element, final FunctionCallEdge callEdge) {
    ...
    protected boolean hasFunctionPointerRecursion(final CallstackState element, final FunctionCallEdge pCallEdge) {
    ...
    protected boolean hasVoidRecursion(final CallstackState element, final FunctionCallEdge pCallEdge) {
