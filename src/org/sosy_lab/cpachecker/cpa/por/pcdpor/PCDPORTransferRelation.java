...
public class PCDPORTransferRelation extends SingleEdgeTransferRelation {
    ...
    @Option(secure = true, description = "With this option enabled, thread creation edge will be regarded as normal edge, which may reduce redundant interleavings.")
    ...
    public PCDPORTransferRelation(Configuration pConfig, LogManager pLogger, CFA pCfa) throws InvalidConfigurationException {
        pConfig.inject(this);
...
//        String s = String.valueOf(pCfaEdge.getLineNumber());
//        System.out.println("line:"+s);

//        FunctionEntryNode mainFunction = edgeInfo.getCfa().getMainFunction();
//        Optional<LiveVariables> numberOfFunctions = edgeInfo.getCfa().getLiveVariables();
//        System.out.println("main:"+mainFunction.getFunctionName());
//        System.out.println("numberofFunction: "+numberOfFunctions);

        // compute new locations.
        Collection<? extends AbstractState> newStates = locationsCPA.getTransferRelation().getAbstractSuccessorsForEdge(curState.getCurrentThreadLocs(), pPrecision, pCfaEdge);
        ...
        if (newStates.isEmpty()) {
            ...
            Pair<Integer, Map<String, Integer>> newThreadIdInfo = updateThreadIdNumber(curState.getCurrentThreadCounter(), oldThreadIdNumbers, newLocs);
            ...
            return ImmutableSet.of(new PCDPORState(new PeepholeState(newThreadCounter, pCfaEdge, newLocs, newThreadIdNumbers), determineEdgeType(pCfaEdge), new HashSet<>(), false));
        }
    }

    private Pair<Integer, Map<String, Integer>> updateThreadIdNumber(int pOldThreadCounter, final Map<String, Integer> pOldThreadIdNumbers, final LocationsState pNewLocs) {
        ...
        ImmutableSet<String> removeTids = from(newThreadIdNumbers.keySet()).filter(t -> !newThreadIds.contains(t)).toSet();
        removeTids.forEach(t -> newThreadIdNumbers.remove(t));

        // add new thread id.
        ImmutableSet<String> addTids = from(newThreadIds).filter(t -> !newThreadIdNumbers.containsKey(t)).toSet();
